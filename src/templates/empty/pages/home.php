...
<br>
<h4>Pool fee</h4>
<h2><?php echo formatPercents($server_configuration['recipientFee'] * 100, $pool_configuration['math_precision']); ?>
</h2>
...
<?php if ($pool_configuration['network_hashrate_multiplier'] != 1) { ?><sub>Warning! This value is estimated</sub><?php } ?>
...
<?php if ($pool_configuration['network_difficulty_multiplier'] != 1) { ?><sub>Warning! This value is estimated</sub><?php } ?>
...
<?php if ($pool_configuration['suggested_software'] != '') { ?>
  <br>
  Download the mining software - we suggest <b><?php echo $pool_configuration['suggested_software']; ?></b> for the
  <b><?php echo $server_configuration['algorithm']; ?></b> algorithm.
  <br>
...
foreach ($ports_current as $port) {
  if ($pool_configuration['suggested_platform_gpu']) { ?>
    ...
    <code class="command"><strong>./<?php echo $pool_configuration['suggested_software_linux']; ?></strong><?php echo $pool_configuration['suggested_command_algo']; ?><b><?php echo $server_configuration['algorithm']; ?></b><?php echo $pool_configuration['suggested_command_open']; ?><b>stratum+tcp://<?php echo getServerVariable('SERVER_NAME') . ':' . $port['port']; ?></b><?php echo $pool_configuration['suggested_command_wallet']; ?><b>&lt;WALLET&gt;</b>
      <?php if ($pool_configuration['suggested_command_worker'] != '') {
        echo '' . $pool_configuration['suggested_command_worker'] . '<b>&lt;WORKER&gt;</b>';
      } ?></code>
    <br>
